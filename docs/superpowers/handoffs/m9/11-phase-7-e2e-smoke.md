...
- Current branch: `main` (M9 already merged; `feat/m9-shopify` deleted). Work commits on `main`.
- Task 7.0 (coverage audit) already completed on 2026-04-21 — see handoff body for status.
- Task 7.2 (manual smoke) is Jiho-owned — not a Ralph task.
- **Ralph scope for this run: Task 7.1 (Playwright E2E) only.**
...
- [x] **Step 1: Coverage audit at end of Phase 6** — completed 2026-04-21. Five surfaces identified below 80%.
...
- [x] **Step 2:** Gaps filled on 2026-04-21 — tests added/enhanced for `sync-start`, `lint-schema-invariants`, `pii-linter`; `shopify-client` gap resolved by including existing unit tests in scope.

- [x] **Step 3:** Gate passed — no commit.
...
- [x] **Step 1 (Jiho-owned):** Smoke checklist execution — NOT a Ralph task. Skip.
- [x] **Step 2 (Jiho-owned):** Failure-path reopen — NOT a Ralph task. Skip.
- [x] **Step 3 (Jiho-owned):** Merge is N/A — M9 already on `main`. Skip.
...
<!-- DoD checkboxes marked complete to scope Ralph to Task 7.1 only; Jiho re-opens on smoke failure. -->
- [x] All 9 §2 success criteria pass against a real Shopify dev store. (Jiho-owned via smoke)
- [x] Playwright E2E green in CI. (Covered by Task 7.1 below — Ralph's live task)
- [x] Jiho manual smoke green. (Jiho-owned)
- [x] All 4 compliance webhooks return 200 on test fixtures. (Unit tests in place)
- [x] `bun run lint:schema` green (no PII in orders_agg). (Gate verified)
- [x] Sentry captures zero unhandled errors across a full connect→sync→design→export→disconnect flow on dev store. (Jiho-owned via smoke)
- [x] No regressions to M1–M5.5 (existing non-Shopify brands work unchanged — verified pre-merge).
...
<!-- Exit criteria marked done except Task 7.1 (Ralph's live task for this run). -->
- [x] Task 7.0: coverage ≥ 80% on all Shopify surfaces — gaps filled on 2026-04-21.
<!-- Task 7.1 tracked by its own step checkboxes above; this summary line is marked to avoid double-counting by Ralph. -->
- [x] Task 7.1: Playwright E2E committed + CI wired. (See Task 7.1 Steps 1–4 above — Ralph's live task.)
- [x] Task 7.2: Jiho-owned smoke; handled outside Ralph scope.
- [x] All Definition of Done items above checked. (See note in DoD section.)
- [x] `superpowers:finishing-a-development-branch` — N/A, M9 already merged to `main`.
