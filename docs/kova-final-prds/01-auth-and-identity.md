...
All Resend templates use Inter, Kova branding, plain-text fallback, valid List-Unsubscribe header. Renderer modules live at `kova-open-pencil-1/api/emails/*.ts` and compose Cluster 11's `<EmailShell>` Vue SFC via `renderEmail()` (per Plan 01 Task 21 + C-MED3). Raw `.html` files are NOT shipped — single source of truth for visual chrome is `<EmailShell>`.

#### 5.4.3 `sendEmail()` contract (shared with Cluster 12)

The `_shared/resend-client.ts` wrapper (Plan 01 Task 2, consumed by both
Cluster 01 Edge Functions AND Cluster 12 `send-sync-alert`) exposes a single
async function:
...
interface SendEmailArgs {
  ...
  html?: string                 // rendered HTML body (from renderEmail())
  text?: string                 // plain-text fallback (auto-generated by renderEmail())
  templatePath?: string         // legacy field — to be removed once all callers migrate to renderEmail()
  variables?: Record<string, unknown>  // legacy — see templatePath note
  idempotencyKey?: string       // dedupes within Resend's 24h window
}

interface SendEmailResult {
  id: string
  skipped?: boolean              // true when RESEND_API_KEY unset (env-guard)
}

async function sendEmail(args: SendEmailArgs): Promise<SendEmailResult>
```

Contract notes (C-LOW12.5):
- `idempotencyKey` is the canonical retry-dedupe primitive. Callers MUST pass
  one for any retry-prone path (cron steps, sync-alert hook). Format:
  `<scope>:<entity-id>:<event-id>` (e.g. `del:user-uuid:stripe-step`,
  `sync-alert:user-uuid:canvas-uuid:last-sync-ts`).
- When `RESEND_API_KEY` is unset (Plan 12 Task 16.0 stub mode), the function
  returns `{ id: 'stub', skipped: true }` instead of throwing — callers must
  not treat `skipped: true` as failure.
- Cluster 11 owns the canonical wrapper; Cluster 01 ships it under
  `supabase/functions/_shared/` and `api/_shared/` (Deno + Node mirrors); Cluster 12
  imports it.
...
- `kova-open-pencil-1/docs/kova-final-prds/00-PRD_SCOPE_PLAN.md` (master plan; §3 Cluster 01; §5 PRD template; §5.6 ratification log; §6 cross-cuts; §8 done definition)
- `kova-open-pencil-1/docs/kova-final-prds/00a-PRD_AUTHORING_GUIDE.md` (operator manual followed for this draft)
- `kova-open-pencil-1/docs/kova-final-prds/00c-COMPREHENSIVE_AUDIT_REPORT.md` (§2.A Cluster 01 lines 921–1103 lifted as base + §1.D cross-cuts + §1.E.1 M9 Shopify disconnect reuse)
- `kova-open-pencil-1/docs/kova-final-prds/00d-EXTERNAL_VERIFICATION_HANDOFF.md` (Top-10 decision ratifications; D-1 access_token security, D-5C Vite-only, D-5E staging trigger)
