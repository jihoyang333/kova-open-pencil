...
**Tech Stack:** Vue 3 (`<script setup lang="ts">` Composition API), Pinia (setup stores), Reka UI (Dialog / Popover / DropdownMenu / Tooltip wrappers), Tailwind CSS 4 (`@theme` token translation), VueUse (`useMediaQuery`, `useLocalStorage`), Supabase JS (`@supabase/supabase-js` Realtime), `@sentry/vue` + `@sentry/node`, Resend SDK, `valibot` (schema), `bun:test` (unit), Playwright (E2E), `juice` (CSS inlining for emails). PRD: `kova-open-pencil-1/docs/prd/11-shared-ui-infrastructure.md`.
