...
- Brand label CLICK → navigates `router.push('/dashboard?brandId=' + selectedBrand.id)` per Q17 (no popover, no in-canvas switching)
- Right: notifications icon (Phase 2 — render as muted/hover-only), Present icon (Phase 2 — placeholder), Comments icon (DEFERRED per Q6/scope plan — render as muted), avatar (initials, 26×26 circle, brand-color background)
...
- Tab strip: **Design** (default-active) + **AI**. Prototype tab NOT rendered (per founder ratification 2026-05-15, scope plan §3 Cluster 06 "Prototype DEFERRED" line is superseded by "Prototype out of scope entirely")
...
**Shop panel (canvas Assets panel — left or right depending on layout per scope plan §3 Cluster 05; per Shopify spec §4.1 it docks in the LEFT sidebar at ~260 px):**
- Per Shopify spec §4.1, the Shop panel UI lives in the LEFT sidebar (alongside Pages + Layers as a separate section/tab, OR as a fold-out from the brand-kit Assets section per scope plan §3 Cluster 05). **Recommendation: render Shop panel as a third left-panel section below Layers, expandable.** See §12 for ESCALATE.
...
| Topbar (`.topbar`) | same | same | lines 100–141 (markup), 611–627 (instance) | 44 px height; left = logo + file breadcrumb (brand pill + sep + file name + caret); right = notifications + present + comments + avatar |
...
| Brand label CLICK behavior | Q17 | `router.push('/dashboard?brandId=' + selectedBrand.id)`. NO popover. (Cluster 02 owns dashboard route.) |
...
| Comments icon | Final.html line 624 | DEFERRED (Q6 / scope plan). Hidden OR visible-disabled (founder confirms — see §12) |
...
| `useRightPanelStore` (NEW) | `src/stores/right-panel.ts` | `activeTab: 'design' \| 'ai'` (NOT 'prototype' — explicitly omitted). Persists per-canvas via `usePreferencesStore` Layer 2 (localStorage), keyed `right-panel-tab:${canvasId}` | `isDesignActive`, `isAiActive` | `setActiveTab(tab: 'design'\|'ai')`, `toggleAi()` |
...
| `FileBreadcrumb` | `src/components/editor/FileBreadcrumb.vue` | `props: { brand, fileName }`, `emits: ['brand-click', 'file-menu-toggle']`. brand-click → `router.push('/dashboard?brandId=...')` per Q17 | Final lines 614–620 |
| `TopChromeActions` | `src/components/editor/TopChromeActions.vue` | Renders right side: Notifications (disabled), Present (disabled), Comments (DEFERRED — see §12), AvatarDropdown anchor | Final lines 622–625 |
...
|---|---|---|---|
| `LeftPanel` | `src/components/editor/LeftPanel.vue` | Composes file-row + Pages section + Layers section. Width 240 px | Final 632–705 |
...
| `ShopPanel` | `src/components/editor/sidebar/ShopPanel.vue` (existing M9 — REFACTOR per Shopify spec §5.2) | Collapses 3-tab TabsRoot into single product grid. Mounts inside LeftPanel as third section below Layers (recommended — see §12) | Shopify spec §4.1 |
...
| `RightPanelTabs` | `src/components/editor/RightPanelTabs.vue` | TWO tabs: Design (default-active) + AI. Reka Tabs (Cluster 11). NO Prototype tab. | Final 811–820 + PRD 10 §3.1 |
...
- [ ] Clicking brand label navigates to `/dashboard?brandId={currentBrandId}` (per Q17); no popover opens
- [ ] Clicking logo dropdown caret opens the File menu (Cluster 08); first item "Back to dashboard" routes to `/dashboard?brandId=...`
...
- [ ] Comments icon either rendered as visible-disabled OR hidden — founder confirms in §12
...
- [ ] Right panel renders exactly TWO tabs: **Design** (default-active) + **AI**
...
- [ ] Shop panel renders inside LeftPanel as a third section below Layers (recommended per §12)
...
| `tests/unit/components/editor/RightPanelTabs.test.ts` | EXACTLY 2 tabs visible (Design + AI); Prototype tab NOT in DOM; default-active = Design; click switches |
...
| `tests/e2e/editor/brand-label-navigates.spec.ts` | Click brand breadcrumb → assert URL is `/dashboard?brandId=...` |
...
|---|---|---|---|
| **12.1** | **Shop panel docks where?** Shopify spec §4.1 says "canvas left sidebar, ~260 px wide." Hi-fi Final.html shows Left panel as Pages + Layers only. Two options: (a) **third left-panel section below Layers** (recommended — preserves existing left-panel structure, expandable section). (b) **separate left rail next to the existing left panel** (matches Shopify spec width exactly but adds new chrome). | MEDIUM | **Recommendation: option (a)** — Shop panel as third left-panel section under Pages + Layers. Width 240 px (matches existing left panel). Expand/collapse with section header. Founder ratifies in §0 review. |
| **12.2** | **`editor.reorderPage` + `editor.duplicatePage` audit** — 03 doc §2.4 Open Q says `useEditorStore` may only have `addPage` + `deletePage` + `renamePage`. Cluster 06's Pages right-click needs `duplicatePage` + `reorderPage`. | MEDIUM | **ESCALATE to Cluster 07a**: confirm + add primitives if missing. If 07a does NOT add them, Cluster 06 implements them as Yjs page-array splice via `editor.ts` (acceptable fallback). |
| **12.3** | **Comments icon in topbar** — DEFERRED per scope plan. Render as visible-disabled OR hide entirely? | LOW | **Recommendation: hide entirely** in MVP. Avoids teasing a feature that has no defined Phase 2 timeline. If founder prefers visible-disabled (consistent with Components tool), switch. Founder confirms in §0. |
...
- **§12.1** — Shop panel: third-left-panel-section vs separate-left-rail. **Recommend (a).**
- **§12.2** — `editor.reorderPage` + `duplicatePage` primitives: confirm Cluster 07a adds them.
- **§12.3** — Comments icon: hide vs visible-disabled. **Recommend hide.**
- **§12.10** — section-priority contract: confirm with Cluster 07b author.
...
- `kova-open-pencil-1/docs/prd/00-PRD_SCOPE_PLAN.md` §3 Cluster 06 (scope) + §6 cross-cuts table
- `kova-open-pencil-1/docs/prd/00c-COMPREHENSIVE_AUDIT_REPORT.md` §2.A Cluster 06 (lines 1551–1613) — auditor's pre-baked spec
- `kova-open-pencil-1/docs/prd/00d-EXTERNAL_VERIFICATION_HANDOFF.md` — D-10 (no PRD 06 split — keep as one)
- `kova-open-pencil-1/docs/prd/00e-EXTERNAL_VERIFICATION_VERDICT.md` §6 (PRD-hygiene notes — no live multi-device sync promises)
- `kova-open-pencil-1/docs/prd/10-ai-chat-and-memory.md` §6 + §11 + §12.1 — Cluster 06 amendment task (right-panel 2-tab framework, Shop panel rework, ChatPopup removal)
...
- `kova-open-pencil-1/docs/prd/01-auth-and-identity.md` — section template, idempotency, RLS shape, acceptance phrasing, test-plan layout
- `kova-open-pencil-1/docs/prd/05-brand-kit-and-drag-drop.md` — MIME contract definitions (consumed by §6.5)
