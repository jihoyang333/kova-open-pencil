...
mock.module('@/stores/auth', () => ({
  useAuthStore: () => ({ user: { id: 'user-42' } }),
}))

let store: ReturnType<typeof getStore> extends Promise<infer T> ? T : never
...
// silence unused-typing warning for the test-only helper
void store
